%% DINAMIKUS C_D (REYNOLDS-FÜGGŐ), ZAJ ÉS SINDy VISSZASZIMULÁLÁS
clear; clc; close all;

% --- 1. Fizikai Paraméterek ---
m = 0.5; g = 9.81; rho = 1.225; D = 0.05; A = (pi * D^2) / 4; mu = 1.81e-5;

% White-féle approximáció
...
%% --- 1.5 ZAJ HOZZÁADÁSA (A "Valóság" szimulálása) ---
% Állítsd ezt az értéket! Kezdjük egy nagyon pici, pl. 0.05-ös zajjal.
zaj_szint = 0.9; 

rng(42); % Fixáljuk a véletlent, hogy a futtatások összehasonlíthatók legyenek
s_zajos = s_valos + zaj_szint * randn(size(s_valos));
v_zajos = v_valos + zaj_szint * randn(size(v_valos));
a_zajos = a_valos + zaj_szint * randn(size(a_valos));

%% --- 2. SINDy ALGORITMUS (A ZAJOS adatokon) ---
a_ismert = g .* ones(size(t_span));
a_maradek = a_zajos - a_ismert; % A SINDy a zajos gyorsulásból dolgozik

% Könyvtár építése a ZAJOS állapotváltozókból
Theta = [ones(size(s_zajos)), s_zajos, v_zajos, s_zajos.*v_zajos, ...
         s_zajos.^2, v_zajos.^2, (s_zajos.^2).*v_zajos, s_zajos.*(v_zajos.^2), ...
         s_zajos.^3, v_zajos.^3]; 

konyvtar_nevek = {'1', 's', 'v', 's*v', 's^2', 'v^2', 's^2*v', 's*v^2', 's^3', 'v^3'};

% SINDy optimalizálás
lambda = 0.0001; % Nagyon kicsi küszöb
Xi = Theta \ a_maradek; 
for k = 1:10
...
%% --- 3. A SINDy MODELL VISSZASZIMULÁLÁSA ---
...
% Szimuláció futtatása a megtalált (zajon tanult) egyenlettel
[~, x_sindy] = ode45(ode_func_sindy, t_span, [0; 0]);
s_sindy = x_sindy(:, 1);
v_sindy = x_sindy(:, 2);

% A becsült gyorsulás kiszámítása a grafikonhoz
...
clc;
fprintf('--- SINDy Által Talált Egyenlet (Zajszint: %.3f, Lambda: %.4f) ---\n', zaj_szint, lambda);
talalt = false;
for i = 1:length(Xi)
    if Xi(i) ~= 0
...
% Ábrázolás
figure('Name', 'Valós vs. Zajos Mérés vs. SINDy', 'Position', [100, 100, 1200, 400]);

subplot(1,3,1);
plot(t_span, s_valos, 'k', 'LineWidth', 2); hold on;
plot(t_span, s_zajos, 'g.', 'MarkerSize', 8);
plot(t_span, s_sindy, 'r--', 'LineWidth', 2);
title('Út (s)'); xlabel('Idő [s]'); ylabel('[m]'); 
legend('Valós', 'Zajos Mérés', 'SINDy (Becsült)', 'Location', 'best'); grid on;

subplot(1,3,2);
plot(t_span, v_valos, 'k', 'LineWidth', 2); hold on;
plot(t_span, v_zajos, 'g.', 'MarkerSize', 8);
...
plot(t_span, a_zajos, 'g.', 'MarkerSize', 8);
